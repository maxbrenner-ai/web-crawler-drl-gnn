...
class FullyConnected_baseline(nn.Module):
    def __init__(self, feat_size, hidden_size, message_size, goal_size, goal_opt, critic_agg_weight,
                 combined_a_c, device):
    ...
    def _aggregate_all(self, messages, num_nodes):
        # Cut up messages
        num_states = len(num_nodes)
        stacks = []
        start_indx = 0
        for N in num_nodes:
            mess = messages[start_indx:start_indx + N]
            start_indx += N
            agg_stack = self._aggregate(mess)
            stacks.append(agg_stack)
        stack = torch.cat(stacks)

        return stack

    # Sum all the messages wor by row, stack up by the number of nodes, then take that and subtract the messages
    # this will give the fc agg
    def _aggregate(self, messages):
        num_rows = messages.shape[0]
        total_sum = torch.sum(messages, dim=0)
        stacked_total_sum = torch.stack([total_sum] * num_rows)
        assert stacked_total_sum.shape == messages.shape
        agg = stacked_total_sum - messages
        return agg
